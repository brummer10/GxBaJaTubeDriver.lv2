...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_bajatubedriverUI * const ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const struct _LV2UI_Descriptor * descriptor,
	...
	gx_bajatubedriverUI* ui = (gx_bajatubedriverUI*)malloc(sizeof(gx_bajatubedriverUI));
	...
	ui->controls[1] = (gx_controller) {{0.15, 0.34, 0.0, 1.0, 0.01}, {380, 55, 81, 81}, false,"DRIVE", KNOB, DRIVE};
	ui->controls[2] = (gx_controller) {{0.5, 0.5, 0.0, 1.0, 0.01}, {480, 55, 81, 81}, false,"TONE", KNOB, TONE};
	ui->controls[3] = (gx_controller) {{0.5, 0.34, 0.0, 1.0, 0.01}, {580, 55, 81, 81}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
	ui->start_value = 0.0;
	ui->sc = NULL;
	ui->set_sc = 0;
	...
	ui->width = ui->init_width -380 + (100 * CONTROLS);
	...
	*widget = (void*)ui->win;
	...
	ui->rescale.xc = (double)ui->width/(ui->init_width -380 + (100 * CONTROLS));
...
static void cleanup(LV2UI_Handle handle) {
	gx_bajatubedriverUI * const ui = (gx_bajatubedriverUI*)handle;
	cairo_destroy(ui->cr);
...
static void knob_expose(const gx_bajatubedriverUI * const ui, const gx_controller * const knob) {
...
static void draw_grid(const gx_bajatubedriverUI * const ui) {
...
static void bypass_expose(const gx_bajatubedriverUI * const ui,  const gx_controller * const switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
...
static void draw_controller(const gx_bajatubedriverUI * const ui,  const gx_controller * const controller) {
	if (controller->type < BSWITCH) knob_expose(ui, controller);
	else bypass_expose(ui, controller);
}

// general XWindow expose callback, 
static void _expose(const gx_bajatubedriverUI * const ui) {
	const char* plug_name = "BaJaTubeDriver" ;
	...
	cairo_text_extents_t extents;
	cairo_set_source_rgba (ui->cr, 0.0, 0.0, 0.0,0.8);
	...
	cairo_move_to (ui->cr, ((double)(ui->width/1.1)/ui->rescale.x-(extents.width)-2.0),
	  ((double)(ui->height/4.0)/ui->rescale.y-extents.height)-1.0);
	cairo_show_text(ui->cr, plug_name);
	cairo_set_source_rgba (ui->cr, 0.6, 0.6, 0.6,0.6);
	cairo_move_to (ui->cr, ((double)(ui->width/1.1)/ui->rescale.x-(extents.width)),
	  (double)(ui->height/4.0)/ui->rescale.y-extents.height);
	cairo_show_text(ui->cr, plug_name);
...
static void controller_expose(const gx_bajatubedriverUI * const ui,  const gx_controller * const control) {
	cairo_push_group (ui->cr);
...
static void send_controller_event(gx_bajatubedriverUI * const ui, int controller) {
...
static void check_value_changed(gx_bajatubedriverUI * const ui, int i, const float * const value) {
...
static void check_is_active(gx_bajatubedriverUI * const ui, int i, bool set) {
...
static bool aligned(int x, int y, const gx_controller * const control, const gx_bajatubedriverUI * const ui) {
...
static bool get_active_ctl_num(gx_bajatubedriverUI * const ui, int *num) {
	bool ret = false;
	for (int i=0;i<CONTROLS;i++) {
...
static bool get_active_controller_num( const gx_bajatubedriverUI * const ui, int *num) {
	for (int i=0;i<CONTROLS;i++) {
...
static void scroll_event(gx_bajatubedriverUI * const ui, int direction) {
	float value;
	int num;
...
static void enum_event(gx_bajatubedriverUI * const ui, int i) {
	float value;
...
static void switch_event(gx_bajatubedriverUI * const ui, int i) {
	float value = ui->controls[i].adj.value ? 0.0 : 1.0;
	check_value_changed(ui, i, &value);
}

// left mouse button is pressed, generate a switch event, or set controller active
static void button1_event(gx_bajatubedriverUI * const ui, double* start_value) {
	int num;
...
static void motion_event(gx_bajatubedriverUI * const ui, double start_value, int m_y) {
...
static void set_key_value(gx_bajatubedriverUI * const ui, int set_value) {
	float value = 0.0;
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void key_event(gx_bajatubedriverUI * const ui, int direction) {
	float value;
	int num;
	if (get_active_controller_num(ui, &num)) {
		value = min(ui->controls[num].adj.max_value,max(ui->controls[num].adj.min_value, 
...
static void set_previous_controller_active(gx_bajatubedriverUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void set_next_controller_active(gx_bajatubedriverUI * const ui) {
	int num;
	if (get_active_controller_num(ui, &num)) {
		ui->controls[num].is_active = false;
		send_controller_event(ui, num);
...
static void get_last_active_controller(gx_bajatubedriverUI * const ui, bool set) {
	int num;
	if (get_active_controller_num(ui, &num)) {
...
static void event_handler(gx_bajatubedriverUI * const ui) {
	XEvent xev;
